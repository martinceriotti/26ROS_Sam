"""
Round 4 — Tabular (Round 5) + PCA CLIP + 5 scores zero-shot.

Mejoras sobre Round 3:
  - 32 componentes PCA de embeddings CLIP
  - 5 scores zero-shot: distress, luxury, renovation, view, pool
    (calculados desde embeddings guardados — sin reprocesar imágenes)
...
Run desde participant/:
    python models/round4_clip_scores.py
"""
...
print("Cargando embeddings CLIP y scores zero-shot...")
...
# Todos los scores zero-shot desde clip_scores.csv
scores_df = pd.read_csv('data/clip_scores.csv').set_index('zpid')
...
print(f"  {emb_matrix.shape[0]:,} embeddings cargados, {len(CLIP_SCORE_COLS)} scores zero-shot")
...
output_path = 'submissions/round4_clip_scores.csv'
...
oof_path = 'submissions/oof_round4_clip_scores.csv'
