"""
Round 3 — Tabular (Round 5) + Embeddings CLIP de imágenes.

Mejoras sobre Round 5:
  - 32 componentes PCA de embeddings CLIP (foto principal de cada propiedad)
  - clip_distress_score: señal visual de deterioro/abandono (zero-shot CLIP)

La PCA se ajusta en train antes del CV (embeddings no usan el target → sin leakage).
Todo lo demás es idéntico a round5_conservative.py.

Requiere haber corrido primero:
    python models/extract_embeddings.py

Run desde participant/:
    python models/round3_images.py
"""
...
output_path = 'submissions/round3_images.csv'
...
oof_path = 'submissions/oof_round3_images.csv'
