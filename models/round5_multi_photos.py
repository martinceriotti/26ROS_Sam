"""
Round 5 — Tabular + embeddings CLIP promediados de hasta 5 fotos por propiedad.

Mejoras sobre Round 4:
  - Embeddings promediados de 5 fotos (vs 1 foto en R3/R4) — representacion mas robusta
  - 5 scores zero-shot recalculados del embedding promedio
  - PCA con 64 componentes (vs 32) — captura mas varianza (83.6% vs 73.4%)

Requiere haber corrido:
    python models/extract_embeddings_multi.py

Run desde participant/:
    python models/round5_multi_photos.py
"""
...
ALPHA           = 0.35
PENALTY_FORECLOSURE = 0.60
LOG_CAP_BUFFER  = np.log(1.0)
N_PCA           = 64
...
print("Cargando embeddings multi-foto y scores zero-shot...")
data_emb   = np.load('data/embeddings_clip512_multi.npz')
emb_zpids  = data_emb['zpids'].astype(int)
emb_matrix = data_emb['embeddings']    # (N_emb, 512) — promedio de hasta 5 fotos

zpid_to_row = {z: i for i, z in enumerate(emb_zpids)}

# Scores zero-shot calculados del embedding promedio
scores_df = pd.read_csv('data/clip_scores_multi.csv').set_index('zpid')
CLIP_SCORE_COLS = ['clip_distress_score', 'clip_luxury_score',
                   'clip_renovation_score', 'clip_view_score', 'clip_pool_score']
zpid_to_scores = scores_df[CLIP_SCORE_COLS].to_dict(orient='index')

n_photos = data_emb['n_photos']
print(f"  {emb_matrix.shape[0]:,} propiedades, promedio {n_photos.mean():.1f} fotos/prop")
print(f"  {len(CLIP_SCORE_COLS)} scores zero-shot cargados")
...
for col in CLIP_SCORE_COLS:
    train[col] = train['zpid'].map(lambda z, c=col: zpid_to_scores.get(z, {}).get(c, 0.5))
    test[col]  = test['zpid'].map(lambda z, c=col: zpid_to_scores.get(z, {}).get(c, 0.5))

IMAGE_FEATURES = IMG_PC_COLS + CLIP_SCORE_COLS
...
print(f'\nEntrenando Round 3 (tabular + imágenes) — Quantile alpha={ALPHA}\n')
print(f'  Features totales: {len(FEATURES_WITH_ZIP)} '
      f'(+{len(IMAGE_FEATURES)} de imagenes vs Round 5: {N_PCA} PCA + {len(CLIP_SCORE_COLS)} scores zero-shot)')
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(train), 1):
    ...
    for seg in ['SF', 'CONDO', 'REST']:
        ...
        # Imprimir importancia de features de imagen (solo fold 1)
        if fold == 1 and seg == 'SF':
            ...
            print(f"  [Fold 1 SF] Importancia features imagen (top 10):")
            for fname, fval in img_fi.head(10).items():
                print(f"    {fname}: {fval}")
...
print(f'\nCap ZIP (P95): {n_oof_capped} OOF | {n_test_capped} test')

# ── Post-processing: penalidades ──────────────────────────────────────────────
...
def apply_distress_penalties(prices, df, label=''):
    ...
    print(f'{label} penalizadas: foreclosure={mask_fc.sum()} props × {PENALTY_FORECLOSURE:.0%}')
    return prices
...
oof_pred_pre   = np.expm1(oof_preds)
oof_pred_post  = oof_prices_pp
...
print(f'  (Round 5 referencia:          ~21-22%   sesgo: ~+2%)')
print(f'{"-"*55}')
...
output_path = 'submissions/round5_multi_photos.csv'
...
oof_path = 'submissions/oof_round5_multi_photos.csv'
