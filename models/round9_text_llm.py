"""
Round 9 — Tabular + Imagenes CLIP + Features LLM de texto
==========================================================
Igual que round3_images.py pero agrega los features extraidos por Claude Haiku
desde las descripciones de propiedades (~8,100 propiedades con contenido real).

Features nuevos (~19 columnas):
  llm_condition, llm_tier, llm_has_view, llm_has_ocean_view, llm_has_bay_view,
  llm_has_city_view, llm_has_golf_view, llm_floor_level, llm_is_penthouse,
  llm_is_corner, llm_has_concierge, llm_has_elevator, llm_has_boat_dock,
  llm_has_guest_house, llm_amenities_count, llm_is_fixer, llm_distress_count,
  llm_renovation_age

Requiere haber corrido primero:
    python models/extract_embeddings.py        (embeddings CLIP)
    python models/extract_text_features.py     (features LLM)

Run desde participant/:
    python models/round9_text_llm.py
"""
...
# Cargar features LLM
llm_features_df = pd.read_csv('data/llm_description_features.csv')
...
emb_matrix = data_emb['embeddings']
...
def add_static_features(df):
    ...
    # Interacciones con features LLM
    if 'llm_condition' in df.columns:
        df['llm_luxury_combo'] = (df['llm_is_penthouse'] + df['llm_has_concierge'] +
                                  df['llm_has_elevator'] + (df['llm_tier'] == 2).astype(int))
    return df
...
FEATURES_BASE = [
    ...
    'llm_luxury_combo',
] + IMAGE_FEATURES + LLM_MODEL_FEATURES
...
n_llm_ok = int(train['llm_ok'].sum()) if 'llm_ok' in train.columns else 0
print(f'\nEntrenando Round 9 (tabular + imágenes + LLM texto) — Quantile alpha={ALPHA}\n')
print(f'  Features totales: {len(FEATURES_WITH_ZIP)} (+{len(LLM_MODEL_FEATURES)} LLM vs Round 3)')
print(f'  Features LLM OK en train: {n_llm_ok:,} / {len(train):,} ({n_llm_ok/len(train)*100:.1f}%)')
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(train), 1):
    ...
    for seg in ['SF', 'CONDO', 'REST']:
        ...
        # Imprimir importancia de features LLM (solo fold 1, seg SF)
        if fold == 1 and seg == 'SF':
            fi = pd.Series(model.feature_importances_, index=seg_features)
            llm_fi = fi[[c for c in LLM_MODEL_FEATURES if c in seg_features]].sort_values(ascending=False)
            if len(llm_fi) > 0:
                print(f'  [Fold 1 SF] Importancia features LLM (top 10):')
                for fname, fval in llm_fi.head(10).items():
                    print(f'    {fname}: {fval}')
...
print(f'  (Round 3 referencia:          ~21%   sesgo: ~+2%)')
print(f'{"-"*55}')
