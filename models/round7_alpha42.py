"""
Round 7 — Tabular + CLIP images, alpha=0.42 (menos conservador que Round 3)

Unico cambio respecto a round3_images.py: ALPHA 0.35 -> 0.42.

Motivacion: en competencia real R4, SAM compro 9.66 props/ronda (muy selectivo)
vs Aragorn 15.36 props/ronda. Aragorn gano con mas volumen aunque con menor
hit rate. Alpha=0.42 busca el punto medio: mas deals sin destruir la precision.

Alpha=0.35 -> too few buys (9.66/ronda), missing good deals
Alpha=0.50 -> too many buys (15.2/ronda), 37% loss rate (round6_surgical)
Alpha=0.42 -> objetivo: ~12-13 props/ronda con hit rate >65%

Requiere haber corrido primero:
    python models/extract_embeddings.py

Run desde participant/:
    python models/round7_alpha42.py
"""
...
print(f'\nEntrenando Round 7 (tabular + imagenes) — Quantile alpha={ALPHA}\n')
print(f'  Features totales: {len(FEATURES_WITH_ZIP)} '
      f'(+{len(IMAGE_FEATURES)} de imágenes vs Round 5)')
...
output_path = 'submissions/round7_alpha42.csv'
...
oof_path = 'submissions/oof_round7_alpha42.csv'
oof_submission.to_csv(oof_path, index=False)
print(f'OOF guardado: {oof_path}  ({len(oof_submission):,} filas)  <- subir al tab Practice')
print('\nProtocolo: comparar en Practice vs round3_images (49.7% ROI baseline).')
print('Si mejora -> subir round7_alpha42.csv a la ronda real.')
print('Si no mejora -> conservar round3_images.csv.')
